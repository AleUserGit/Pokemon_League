# Usar LISTAS DE LISTAS, no listas paralelas
# O las funciones van a ser imposibles
# Actualizar las categorías

##### lista de emi  y chat
# lst_pokemones([nombre, tipo, nivel, poder, entrenador, victorias, estado])

lst_pokemones = [
    ["Bulbasaur", "Planta", 45, 49, 49, 45],
    ["Ivysaur", "Planta", 60, 62, 63, 60],
    ["Venusaur", "Planta", 80, 82, 83, 80],

    ["Charmander", "Fuego", 39, 52, 43, 65],
    ["Charmeleon", "Fuego", 58, 64, 58, 80],
    ["Charizard", "Fuego", 78, 84, 78, 100],

    ["Squirtle", "Agua", 44, 48, 65, 43],
    ["Wartortle", "Agua", 59, 63, 80, 58],
    ["Blastoise", "Agua", 79, 83, 100, 78],

    ["Pikachu", "Eléctrico", 35, 55, 40, 90],
    ["Raichu", "Eléctrico", 60, 90, 55, 110],

    ["Jigglypuff", "Normal/Hada", 90, 45, 20, 20],
    ["Meowth", "Normal", 40, 45, 35, 90],

    ["Psyduck", "Agua", 50, 52, 48, 55],
    ["Machop", "Lucha", 70, 80, 50, 35],

    ["Geodude", "Roca/Tierra", 40, 80, 100, 20],
    ["Gastly", "Fantasma/Veneno", 30, 35, 30, 80],

    ["Onix", "Roca/Tierra", 35, 45, 160, 70],

    ["Cubone", "Tierra", 50, 50, 95, 35],

    ["Magikarp", "Agua", 20, 10, 55, 80],
    ["Gyarados", "Agua/Volador", 95, 125, 79, 81],

    ["Snorlax", "Normal", 160, 110, 65, 30],

    ["Articuno", "Hielo/Volador", 90, 85, 100, 85],
    ["Zapdos", "Eléctrico/Volador", 90, 90, 85, 100],
    ["Moltres", "Fuego/Volador", 90, 100, 90, 90],

    ["Dratini", "Dragon", 41, 64, 45, 50],
    ["Dragonair", "Dragon", 61, 84, 65, 70],
    ["Dragonite", "Dragon/Volador", 91, 134, 95, 80],

    ["Mewtwo", "Psíquico", 106, 110, 90, 130],
    ["Mew", "Psíquico", 100, 100, 100, 100]
]
